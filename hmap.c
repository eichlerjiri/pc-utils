...
static void hmap_remove_do(struct hmap_item *prev, struct hmap_item *item) {
	if (item->next) {
...
static void hmap_remove_direct(struct hmap *m, struct hmap_item *prev, struct hmap_item *item) {
	c_free(item->key);
	c_free(item->value);
